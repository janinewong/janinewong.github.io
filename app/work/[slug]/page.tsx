...
import { ManageProblemGoalSection, MultichannelConnectingSection } from "@/components/CaseStudySections";
...
export default async function CaseStudyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className={study.className} style={{ "--case-metric-color": study.colors.metricColor } as CSSProperties}><Header />{study.sections.map((section, index) => {
    ...
    if (section.kind === "manage-problem-goal") return <ManageProblemGoalSection key={index} />;
    if (section.kind === "multichannel-connecting") return <MultichannelConnectingSection key={index} />;
